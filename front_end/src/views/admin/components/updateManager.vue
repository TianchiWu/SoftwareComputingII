        ...
        <a-form :form="form" style="margin-top: 30px">
            <a-form-item label="邮箱" :label-col="{ span: 4 }" :wrapper-col="{ span: 8, offset: 1 }">
                <a-input
                        ...
                        v-if="modify"
                />
                <span v-else>{{ activeManagerInfo.email }}</span>
            </a-form-item>
            <a-form-item label="密码" :label-col="{ span: 4 }" :wrapper-col="{ span: 8, offset: 1 }">
                <a-input
                        ...
                        v-if="modify"
                />
                <span v-else>{{ activeManagerInfo.password}}</span>
            </a-form-item>
    ...
    export default {
        ...
        data() {
            return {
                ...
                form: this.$form.createForm(this, { name: 'updateManager' }),
            }
        },
        computed: {
            ...mapGetters([
        ...
        methods: {
            ...
            cancel() {
                this.set_updateManagerVisible(false)
                this.modify=false
            },
            modifyInfo() {
                setTimeout(() => {
                    this.form.setFieldsValue({
                        'email': this.activeManagerInfo.email,
                        'password': this.activeManagerInfo.password,
                    })
                }, 0)
                this.modify = true
            },
            ...
            handleSubmit(e) {
                this.form.validateFields((err, values) => {
                    if (!err) {
                        ...
                        this.updateManagerInfo(data).then(()=>{
